...
<transition name="menu">
  ...
  <div class="span-wraper" :class="{'isselectedall':isselectAll}" @click="selectAll">
  ...
  <div class="span-wraper" @click="test">
...
watch:{
    shelfselected:{
      ...
      num==0?this.isselectAll=false:(num==this.shelfselected.length?this.isselectAll=true:this.isselectAll=false)
      }
    }
},
methods:{
    test(){
        this.$store.commit('storehome/test')
    },
   
    bookmove(){
         if(this.shelfselected[0]){
             this.popup({
               title:this.shelfselected[1]?`将所选共<span class="shelfpanel-popup-props-html">${this.shelfselected.length}</span>本书移出书架吗?`:`将所选书籍<span class="shelfpanel-popup-props-html">${this.shelfselected[0]}</span>移出书架吗?`,
               confirmText:'确认',
               isRemoveText:true,
               cancelText:'取消',
    ...
    bookconfirmremove(){
       ...
       let tempsearchlist2= tempsearchlist.filter(indxtitle=>{
        ...
        return item.type==1?this.shelfselected.indexOf(item.title)<0:item
        }))
        this.toast({text:'已将'+(this.shelfselected[1]?`您选中的共计<span class="shelfsearch-tip-toast">${this.shelfselected.length}</span>本书成功移出书架!`:`您选中的<span class="shelfsearch-tip-toast">${this.shelfselected[0]}</span>成功移出书架!`)}).show()
       
                
    //    更新3：删除之后下标变化 开始转化
    let tempindexlist=[]
    this.shelflist.forEach((item,index) => {
    tempsearchlist2.indexOf(item.title)<0?'':tempindexlist.push(index)
    });
    ...
    selectAll(){
        this.shelflist.forEach(item=>{
            if(item.type==1&&this.shelfselected.indexOf(item.title)<0&&item.isshow){
